...
class UserFriendshipTest < ActiveSupport::TestCase
	...
	test "that creating a friendship works withput raising an exception" do
		assert_nothing_raised do
			UserFriendship.create user: users(:alex), friend: users(:tony)
		end
	end
	...
	context "a new instance" do
		setup do
			@user_friendship = UserFriendship.new user: users(:alex), friend: users(:tony)
		end
	...
	context "#send_request_email" do
		setup do
			@user_friendship = UserFriendship.create user: users(:alex), friend: users(:tony)
		end
	...
	context "#mutual_friendship" do
		setup do
			UserFriendship.request users(:alex), users(:tony)
			@friendship1 = users(:alex).user_friendships.where(friend_id: users(:tony).id).first
			@friendship2 = users(:tony).user_friendships.where(friend_id: users(:alex).id).first
		end
	...
	context "#accept_mutual_friendship!" do
		...
		should "accpet the mutual friendship" do
			friendship1 = users(:alex).user_friendships.where(friend_id: users(:tony).id).first
			friendship2 = users(:tony).user_friendships.where(friend_id: users(:alex).id).first
	...
	context "#accpet!" do
		setup do
			@user_friendship = UserFriendship.request users(:alex), users(:tony)
		end
		...
		should "include the friend in the list of friends" do
			...
			assert users(:alex).friends.include?(users(:tony))
		end
