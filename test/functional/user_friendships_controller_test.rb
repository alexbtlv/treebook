...
class UserFriendshipsControllerTest < ActionController::TestCase
  ...
  context "#new" do
  	...
  	context "when logged in" do
  		...
  		should "display the friends name" do
  			get :new, friend_id: users(:tony)
  			assert_match /#{users(:tony).full_name}/, response.body
  		end
  		...
  		should "assign a new user friendship correct friend" do
  			get :new, friend_id: users(:tony)
  			assert_equal users(:tony), assigns(:user_friendship).friend
  		end

  		should "assign a new user friendship tot hte currently logged in user" do
  			get :new, friend_id: users(:tony)
  		...
  		should "ask if ou really want to friend the user" do
  			get :new, friend_id: users(:tony)
  			assert_match /Do you really want to friend #{users(:tony).full_name}?/, response.body
  		end
  	end
  end
  ...
  context "#create" do
  	...
  	context "when logged in" do
  		...
  		context "with no friend_id" do
  			...
  			should "redirect to site root path" do
  				assert_redirected_to root_path
  			end
  		end

      context "successfully" do
        should "create two user friendships objects" do
          assert_difference "UserFriendship.count", 2 do
            post :create, user_friendship: { friend_id: users(:tony).profile_name }
          end
        end
  		...
  		context "with a valid friend_id" do
  			...
  			should "redirect to the profile page of the friend" do
  				assert_response :redirect
  				assert_redirected_to profile_path(:mike)
  			end
  ...
  context "#accept" do
    ...
    context "when logged in" do
      setup do
        @friend = create(:user)
        @user_friendship = create(:pending_user_friendship, user: users(:alex), friend: @friend)
        create(:pending_user_friendship, friend: users(:alex), user: @friend)
        sign_in users(:alex)
