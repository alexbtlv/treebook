...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should be logged in to post status" do
    post :create, status: {content: "helo"}
    assert_response :redirect
    assert_redirected_to new_user_session_path
  end
  ...
  test "should create activity item for the status when logged in" do
    sign_in users(:alex)
  ...
  test "should create an activity when status is updated" do
    sign_in users(:alex)
    assert_difference 'Activity.count' do
      put :update, id: @status, status: { content: @status.content }
    end
  ...
  test "should update status for current user when logged in" do
    sign_in users(:alex)
    put :update, id: @status, status: { content: @status.content, user_id: users(:tony).id }
